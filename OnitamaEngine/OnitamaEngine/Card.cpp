...
std::vector<std::vector<char>> Card::GetGridRed()
{
	...
	for(int i=0;i<_jumpOffsetOption.size(); i++)
	{
		int xo = _jumpOffsetOption[i].GetX();
		int yo = _jumpOffsetOption[i].GetY();
		grid[2 + yo][2 + xo] = 'o';
	}
	return grid;
}
...
std::vector<std::vector<char>> Card::GetGridBlue()
{
	...
	for (int i = 0; i < _jumpOffsetOptionRotated.size(); i++)
	{
		int xo = _jumpOffsetOptionRotated[i].GetX();
		int yo = _jumpOffsetOptionRotated[i].GetY();
		grid[2 + yo][2 + xo] = 'o';
	}
	return grid;
}
